...
SERPAPI_KEY = "bf7aa54be98d4580e38369387636cc4b213501d99771cfc95f9d3b704aab4e3e"
domain = "ultravelozmente.com"
workspace_dir = "/Users/hermes/Downloads/ultravelozmentee-main 4"

# Mapa de 5 palabras clave por página para alcanzar > 150 palabras clave en total (36 páginas * 5 = 180 palabras clave)
page_keywords = {
    "index.html": [
...
total_kw_count = sum(len(v) for v in page_keywords.values())
print(f"=== INICIANDO AUDITORÍA PROFUNDA CON SERPAPI ===")
print(f"Páginas a evaluar: {len(page_keywords)}")
print(f"Total Palabras Clave / Frases de Búsqueda a investigar: {total_kw_count}\n")
...
for page_file, kws in page_keywords.items():
    ...
    for kw in kws:
        ...
        try:
            ...
            page_record["keywords_results"].append(kw_data)
            print(f"[{kw_counter}/{total_kw_count}] [{page_file}] '{kw}' -> Rank: {rank if rank else 'Fuera de Top 20'}")
            
        except Exception as e:
            print(f"[{kw_counter}/{total_kw_count}] Error en '{kw}': {e}")
...
print(f"\n✅ Auditoría profunda completada para {total_kw_count} palabras clave. Archivo guardado en {out_file}.")
