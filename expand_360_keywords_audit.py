...
# 5 NUEVAS palabras clave adicionales por página (Long-Tail, Preguntas de Usuarios e Intención Transaccional)
new_page_keywords = {
    "index.html": [
...
total_new_kws = sum(len(v) for v in new_page_keywords.values())
print(f"=== INICIANDO SEGUNDA FASE DE INVESTIGACIÓN PROFUNDA ===")
print(f"Nuevas palabras clave a evaluar: {total_new_kws}\n")
...
for page_file, kws in new_page_keywords.items():
    ...
    for kw in kws:
        ...
        try:
            ...
            print(f"[{counter}/{total_new_kws}] [{page_file}] '{kw}' -> Rank: {rank if rank else 'Fuera de Top 20'}")
            
        except Exception as e:
            print(f"[{counter}/{total_new_kws}] Error en '{kw}': {e}")
            p_record["new_keywords_results"].append({
                "keyword": kw,
                "rank": "Error",
                "error": str(e)
...
print(f"\n✅ Segunda fase completada ({total_new_kws} nuevas palabras clave). Guardado en {out_file}.")
